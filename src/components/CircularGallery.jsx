...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const baseWidth = isMobile ? 640 : 1120;
    const baseHeight = isMobile ? 360 : 630;
...
class App {
  ...
  onClick(clientX, clientY) {
    if (!this.container || !this.medias || !this.viewport) return;
    const rect = this.container.getBoundingClientRect();
    const relativeX = clientX - rect.left;
    const relativeY = clientY - rect.top;
    const worldX = (relativeX / rect.width - 0.5) * this.viewport.width;
    const worldY = (0.5 - relativeY / rect.height) * this.viewport.height;

    const clicked = this.medias.find((media) => {
      if (!media.plane) return false;
      const halfW = media.plane.scale.x / 2;
      const halfH = media.plane.scale.y / 2;
      return (
        worldX >= media.plane.position.x - halfW &&
        worldX <= media.plane.position.x + halfW &&
        worldY >= media.plane.position.y - halfH &&
        worldY <= media.plane.position.y + halfH
      );
    });
